package com.testopah.starwars.client.service;
...
public class PeopleClientService {
	...
	public List<String> buscaPersonagemPorNome() {
		...
		try {
			...
			log.info("Fazendo a busca do personagem na API SWAP...");
			ResponseEntity<PeopleFilter> people = restTemplate.exchange(
					resourceUri, HttpMethod.GET, geraHeader(),
					PeopleFilter.class);
			log.info("Busca concluida com sucesso!");

			List<PeopleFilter> peoples = Arrays.asList(people.getBody());
	...
	public List<String> Listarpersonagens() {
		try {
			...
			ResponseEntity<PeopleFilter> personagens = restTemplate.exchange(
					resourceUri, HttpMethod.GET, null,
					PeopleFilter.class);
			log.info("Busca finalizada com sucesso!");

			List<PeopleFilter> peoples = Arrays.asList(personagens.getBody());
